...
# Memory-aware digest generation prompt
DIGEST_WITH_MEMORY_PROMPT_TEMPLATE = """You are an expert content analyst specializing in social media intelligence with access to historical context. Your task is to analyze a collection of tweets/posts about "{topic}" and create a comprehensive, insightful digest that connects new information with past knowledge.
...
1. **Identifies Key Themes**: What are the main topics, trends, or discussions?
2. **Highlights Important Developments**: What significant news, announcements, or breakthroughs occurred?
3. **Connects to Historical Context**: How do current posts relate to past events or discussions?
4. **Summarizes Community Sentiment**: What is the overall tone and opinion of the community?
5. **Extracts Actionable Insights**: What valuable information can be derived?
6. **Identifies New Facts**: What NEW atomic facts should be remembered for future analysis?
...
{{
  ...
  "new_facts": [
    "Atomic fact 1 (standalone sentence with concrete information)",
    "Atomic fact 2 (standalone sentence with concrete information)"
  ]
}}
```

## Guidelines for Digest
...
- Extract ONLY facts that are NEW (not already in historical context)
- Each fact should be a standalone sentence (e.g., "DeepSeek released V3 model on Jan 15, 2025")
- Focus on concrete events, announcements, or developments
- Return empty array if nothing new to remember
- Avoid opinions, speculation, or redundant information
- Facts should be atomic and self-contained (no pronouns or unclear references)

Analyze the posts carefully and provide your JSON response:"""


def build_digest_prompt_with_memory(
    topic: str,
    time_window_start: datetime,
    time_window_end: datetime,
    posts: list[dict],
    ...
    """
    Build the memory-aware digest generation prompt.
...
You have access to relevant facts from past digests:

{chr(10).join(f"- {fact}" for fact in historical_context)}

Use this context to:
- Identify if current posts are updates to past events
- Recognize recurring themes or patterns
- Provide continuity in your analysis
- Avoid extracting facts that are already known
"""

    return DIGEST_WITH_MEMORY_PROMPT_TEMPLATE.format(
        topic=topic,
        time_window_start=time_window_start.strftime("%Y-%m-%d %H:%M"),
        time_window_end=time_window_end.strftime("%Y-%m-%d %H:%M"),
        total_items=total_items,
        posts=posts_section,
        historical_context_section=context_section
    )
